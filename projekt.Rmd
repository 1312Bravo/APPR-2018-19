...
Vidimo da prevladuje Francija z desetimi, sledijo Španija s sedmimi ter Srbija in Nemčija s petimi.
...
ggplot() + geom_polygon(data=left_join(Evropa, fiba.lestvica.plot.population[,c(1,5)] %>% filter(St.IgralcevNa10milijonov > 0) , by=c("NAME"="Country")),
  ...
  ggtitle("Zemljevid števila NBA igralcev v posamezni evropski državi na 20 milijonov prebivalcev") + xlab("") + ylab("") +
  guides(fill=guide_colorbar(title="Število igralcev")) +
  scale_fill_gradient2(low = "black", high = "yellow", mid = "red", midpoint = 30)
```



```{r zemljevid, echo=FALSE, fig.align='center', warning=FALSE, fig.cap='Graf prikazuje povezavo med plačo in številom točk na minuto, ki jih dosegajo igralci. Dodanih je tudi 5 najbolje plačanih NBA igralcev, ki so vsi američani, ter koliko točk na minuto bi morali dosegati glede na evropske standarde.'}
ggplot(data=tockevsplaca, aes(x=Tocke, y=Placa)) + 
  ...
  ggtitle("Povezava med plačo in točkami") + 
  ylab("Plača") + xlab("Točke") +
  theme_bw()
```


Vidimo, da glede na točke najbolših 5 igralcev nima previsoke plače, ravno nasprotno,
dosegajo večje število točk kot bi jih morali po evropskih standardih, da bi upraviči svojo plačo.
Najbolje plačani igralec je Stephen Curry, ki dosega 0.8253 točke na minuto, drugi je Lebron James, ki dosega 0.7439 točke na minuto, kar je veliko več kot bi morala po evropskih standardih. Tretji je Paul Millsap s 0.4856 točkami na minuto kar pa je posledica tipa igralca, najverjetneje pa je pri njem plača res nekoliko prevelika glede na učinek.


Za konec pa si oglejmo še obnašanje več spremenljivk glede na plačo.
...
ggplot() + geom_smooth(data=metvsplaca.rank, aes(x=EffectiveFieldGoal.rank, y=Salary.rank, col="Met"), se=FALSE, size=2) +
  ...
  labs(title="Obnašanje več spremenljivk glede na plačo, upoštevamo ranking ranking") +
  theme_bw()
```
